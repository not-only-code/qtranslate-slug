...
function qts_section_fn($page_section = false) {
	...
	switch ($page_section['id']) {
		...
		case 'sluglogic':
			...
			printf( '<p>%s</p>', __( 'When saving a post, QTS will try to create the slugs automatically, according to the following rules:', 'qts' ) );
			break;
	}
}
...
function qts_validate_options($input) {
	...
	$sluglogic_options = $settings_output['qts_page_sluglogic'];

	$slugoptions = $settings_output['qts_page_fields'];

    $options = array_merge($styleoptions,$slugoptions, $sluglogic_options );
